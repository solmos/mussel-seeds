...
coast_codes
coast_map <- filter(galicia_map, CODIGO %in% coast_codes$CODIGO)
plot(coast_map)
plot(coast_map[1])
```
...
```{r}
class(coast_map$CODIGO)
class(costatotal$cof)
coast_mussel_sf <- left_join(coast_map, costatotal, by = c(CODIGO = "cof"))
```
...
mus_per_code
muss_sf <- left_join(coast_map, mus_per_code, by = c(CODIGO = "cof"))
plot(muss_sf["mus"])
```

```{r}
library(lme4)
glmer(mus ~ scale(rumbo) + scale(bar) + (1|cof), family = poisson(), data = costatotal)
```
